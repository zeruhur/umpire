...
function diceInstruction(kept: number): string {
  if (kept === 6) return "The desired outcome occurs (-> line). Add one consequence line describing something especially good that also happens for the Actor.";
  if (kept >= 4) return "The desired outcome occurs (-> line). Add a consequence line only if a notable side effect is relevant; otherwise omit it.";
  if (kept >= 2) return "The action proceeds but the outcome is worse than desired (-> line reflects the worse result). Add one consequence line describing the cost or complication.";
